...
class HrExpenseSheet(models.Model):
    ...
    def _check_advance_expense(self):
        for record in self:
            advance_lines = record.expense_line_ids.filtered('advance')
            if record.advance_sheet_id and advance_lines:
                raise ValidationError(_('Advance clearing must not contain any '
                                        'advance expense line'))
            if advance_lines and len(record.expense_line_ids) != 1:
    ...
    def _compute_residual(self):
        for record in self:
            ...
            emp_advance = record.env.ref('hr_expense_advance_clearing.'
                                       'product_emp_advance')
            for line in record.sudo().account_move_id.line_ids:
                if line.account_id == emp_advance.property_account_expense_id:
                    residual_company += line.amount_residual
            record.residual = residual_company
