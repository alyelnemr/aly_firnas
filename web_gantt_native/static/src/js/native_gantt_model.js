...
return AbstractModel.extend({
    ...
    _do_load: function () {
        ...
        // to handle dates groupby ex(create_date:year)
        for(var i = 0; i < n_group_bys.length; i++){
            n_group_bys[i] = n_group_bys[i].split(':')[0];
        }
    ...
    on_data_loaded_ghost: function(tasks, group_bys) {
        ...
        if (ghost_model) {
            var fields_s;
            if (ghost_date_end === null) {
                fields_s = _.uniq([ghost_id ,ghost_name, ghost_date_start, ghost_date_end, ghost_durations]);
            }
            else {
                fields_s = _.uniq([ghost_id ,ghost_name, ghost_date_start, ghost_durations]);
            }
            return this._rpc({
                    model: ghost_model,
                    method: 'search_read',
